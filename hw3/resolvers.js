...
import {
    albums as albumsCollection, artists,
...
export const resolvers = {
    Query: {
        artists: async () => {
            ...
            if (exists) {
                ...
                await client.json.set('Allartists', '$', JSON.stringify(allArtists));
        ...
        companyByFoundedYear:
            async (_, args) => {
                ...
                if (exists) {
                    ...
                    await client.json.set(`company-range-${min}-${max}`, '$', allCompanies);
            ...
            async (_, args) => {
                ...
                if (exists) {
                    ...
                    for (let songId of album.songs) {
                        let exists = await client.exists(`songs-${songId.toString()}`)
                        if(exists){
                            let song = await client.json.get(`songs-${songId.toString()}`);
                            allSongs.push(song);
                        }else{
                            let song = await songs.findOne({_id: new ObjectId(songId)});
                            await client.json.set(`songs-${songId.toString()}`, '$', song);
                            allSongs.push(song);
                        }
                    }
    ...
    Mutation: {
        addArtist: async (_, args) => {
            ...
            let members = args.members.map(member => checkArray(member));
            const newArtist = {
                _id: new ObjectId(),
                name: name,
        ...
        editArtist:
            async (_, args) => {
                ...
                if (args.members && args.members.length > 0) {
                    updateArtist.members = args.members.map(member => checkArray(member));
                }
        ...
        removeCompany:
            async (_, args) => {
                ...
                for (const albumId of company.albums) {
                    const album = await albums.findOne({_id: albumId});
                    ...
                    await client.json.del(`album-${albumId.toString()}`);
                }
        ...
        editAlbum:
            async (_, args) => {
                ...
                if (args.artistId) {
                    ...
                    if (exists.artistId !== artistId) {
                        ...
                        if (!removeAlbumOnOldArtist.acknowledged || removeAlbumOnOldArtist.matchedCount !== 1) {
                            ...
                            await client.json.set(`artist-${exists.artistId}`, '$', oldArtist);
                        }
                ...
                if (args.companyId) {
                    ...
                    if (exists.recordCompanyId !== companyId) {
                        ...
                        if (!removeAlbumInOldCompany.acknowledged || removeAlbumInOldCompany.matchedCount !== 1) {
                            ...
                            await client.json.set(`artist-${exists.recordCompanyId}`, '$', oldCompany);
                        }
                        ...
                        if (!addAlbumInNewCompany.acknowledged || addAlbumInNewCompany.matchedCount !== 1) {
                            ...
                            await client.json.set(`artist-${companyId}`, '$', newCompany);
                        }
                ...
                await client.json.set(`album-${id}`, '$', JSON.stringify(album));
                return album;
            },
        ...
        addSong:
            async (_, args) => {
                ...
                await client.json.set(`song-${id}`, '$', newSong);
                return newSong;
            },
                    ...
                    if (exists.albumId.toString() !== albumId) {
                        ...
                        if (!removeSongInOldAlbum.acknowledged || removeSongInOldAlbum.matchedCount !== 1) {
                            throw new GraphQLError(
                                ...
                                await client.json.set(`song-${exists.albumId}`, '$', oldAlbum);
                        }
