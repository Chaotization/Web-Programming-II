...
export const checkArray = (elem) => {
    if (!Array.isArray(elem)) {
        throw new GraphQLError('Members must be an array', {
            extensions: { code: 'BAD_USER_INPUT' }
        });
    }

    const members = elem.map(member => {
        if (typeof member !== 'string') {
            throw new GraphQLError('Member names must be strings', {
                extensions: { code: 'BAD_USER_INPUT' }
            });
        }

        if (!/^[a-zA-Z]*$/.test(member.trim())) {
            throw new GraphQLError('Member names must contain only alphabetic characters (a-z) or be empty', {
                extensions: { code: 'BAD_USER_INPUT' }
            });
        }
        return member.trim();
